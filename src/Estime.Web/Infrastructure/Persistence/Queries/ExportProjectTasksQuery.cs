...
{
	public class ExportProjectTasksQuery : Query<IEnumerable<TaskExportViewModel>>
	{
		private readonly Guid _projectId;

		public ExportProjectTasksQuery(Guid projectId)
		{
		...
		public override IEnumerable<TaskExportViewModel> Execute()
		{
			...
			var tasks = Session.QueryOver<Task>()
				...
				.Where(x => x.Project.Id==_projectId)
				.SelectList(list => list
					...
					.Select(Projections.Constant(0)).WithAlias(() => alias.Price)
			...
			var wares = Session.QueryOver<Task>()
				...
				.SelectList(list => list
					...
					.Select(() => taskWareAlias.Price).WithAlias(() => alias.Price)
					.Select(x => x.Mileage).WithAlias(() => alias.Mileage)
